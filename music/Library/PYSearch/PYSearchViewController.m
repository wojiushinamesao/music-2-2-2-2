...
- (UITableView *)baseSearchTableView
{
    if (!_baseSearchTableView) {
        UITableView *baseSearchTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT-TOP_HEIGHT-TAB_BAR_HEIGHT) style:UITableViewStyleGrouped];
...
- (void)setup
{
    ...
    UIBarButtonItem * cancleItm = [[UIBarButtonItem alloc] initWithTitle:@"取消" style:UIBarButtonItemStyleDone target:self action:@selector(cancelDidClick)];
    cancleItm.tintColor = COLOR_BLACK128;
    self.navigationItem.rightBarButtonItem = cancleItm;
